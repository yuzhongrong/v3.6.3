 ...
 */
public class UseridKeyboardView extends View {
	private Bitmap mCancelBitmap = UtilsTools.getBitmap(getContext(), R.drawable.close);
	...
	private Bitmap mWhiteBitmap = UtilsTools.getBitmap(getContext(), R.drawable.delete_white);
	private Bitmap mDarkBitmap = UtilsTools.getBitmap(getContext(), R.drawable.delete_dark);
	...
	public UseridKeyboardView(Context context) {
		super(context);
	}

	public UseridKeyboardView(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray Array = initAttrs(context, attrs);
		Array.recycle();
	}

	public UseridKeyboardView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void DrawNumber(Canvas canvas) {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 3; j++) {
				if (i == 3 && j == 0) {
					mSelectPaint.reset();
					mSelectPaint.setColor(mNumberColor);
					mSelectPaint.setAntiAlias(true);
					mSelectPaint.setStyle(Paint.Style.FILL);
					canvas.drawText("X",
							startX * j + startX / 2 - mPaint.measureText("X")
									/ 2, mTopHeight + mPasswordHeight
									+ mNumberHeight * i + mNumberHeight / 2
	...
	private void ActionDown(Canvas canvas) {
		if ((int) downY > (mTopHeight + mPasswordHeight)) {
			...
			if (i == 3 && j == 2) {
				...
				mPaint.setTextSize(mNumberSize);
				if (i == 3 && j == 0) {
					canvas.drawText("X", mNumberWidth * j + mNumberWidth / 2
							- mPaint.measureText("X") / 2,
							mTopHeight + mPasswordHeight + mNumberHeight * i
									+ mNumberHeight / 2
									- (mPaint.ascent() + mPaint.descent()) / 2,
							mPaint);
					...
					canvas.drawText(
							...
							mNumberWidth * j + mNumberWidth / 2
									- mPaint.measureText("" + mNumbers[i][j])
									/ 2, mTopHeight + mPasswordHeight
									+ mNumberHeight * i + mNumberHeight / 2
									- (mPaint.ascent() + mPaint.descent()) / 2,
							mPaint);
