...
public class OnekeyShare implements PlatformActionListener, Callback {
	...
	public static final int SHARE_WEEK = 14;
	public static final int SHARE_INVITE = 15;
		...
		case MSG_ACTION_CCALLBACK: {
			...
			case 1: {
				...
				if (PreferencesUtils.Keys.SHARE_INIT) {
					UmUtils.customEventResult(context, platform.getName(),
	...
	// String uid = PreferencesUtils.getValueFromSPMap(context,
	// PreferencesUtils.Keys.UID);
	// // MyLog.d("TAG", "用户编号：" + uid);
	// params.put("uid", uid);
	// params.put("platform", "4");
	// MyhttpClient.post(UrlConfig.APPHB_SHARE, params,
	...
	// MyLog.i("周分享返回", response);
	// JSONObject obj = new JSONObject(response);
	// int code = obj.getInt("code");
	// if (code == 10000) {
	// MyLog.d("TAG", "红包获取成功");
	// } else {
	// MyLog.d("TAG", obj.getString("msg"));
	// }
	//
