...
import com.android.jnet.net.JsonCallback;
...
/**
 * Created by yuzhongrong on 2017/5/3.
 */

public abstract class CommonJsonCallback<T>  extends JsonCallback<T>{


    public CommonJsonCallback(String url, JSONObject jsonObject) {
        super(url, jsonObject);
    }

    @Override
    public T subConvertSuccess(Response response, String cipher) {
        /*********转换**********/
        ...
        if (typeArgument == Void.class) {
            ...
            T t = Convert.fromJson(cipher, rawType);
            response.close();
            return t;
        } else if (rawType == BaseModel.class) {
            //有数据类型，表示有data
            BaseModel<T> tBaseModel = Convert.fromJson(cipher, type);
            response.close();
